...
DigestAlgo="sha512"
ValidDaysClient="390" # < 13 Months - the final cert in the chain is only valid for < 13 months.
ValidDaysServer="2920" # this is the INTERM CA cert - 2920 Days = 8 Years

currDate=$(date --iso-8601) # Get date keys and cert request is being made - format ISO-8601 e.g., 2023-07-30

CA_KEY="PRIVATE.INTERM-CA.KEY.${Server}.CRYPT.pem"
CA_Cert="${ServerFQDN}.INTERM-CA.FullChainCert_ValidFor${ValidDaysServer}.pem"

RandFile="randfile_${ServerFQDN}_${currDate}.rand"

SignedCertificate="${ClientFQDN}.SignedCertificate_${currDate}_ValidFor${ValidDaysClient}.pem" # <- This goes back to customer
FullChainCertificate="${ClientFQDN}.FullChainCert_ValidFor${ValidDaysClient}.pem" # <- This goes back to customer
...
for i in {1..869};do openssl rand -writerand randfile.rand${i} -base64;done 
for i in {1..869};do cat randfile.rand${i} >> ${RandFile};done
for i in {1..869};do rm randfile.rand${i};done
